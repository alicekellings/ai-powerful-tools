...
const Hero = ({ product }: HeroProps = {}) => {
  // Use product-specific data if provided, otherwise use default AppConfig
  const heading = product ? product.title : AppConfig.title;
  const description = product ? product.description : AppConfig.description;
    ...
    <header className="bg-gradient-to-b from-white to-gray-50 pb-10 pt-16 text-center">
      <Section yPadding="pt-0 pb-0">
        <h1 className="mb-6 text-4xl font-black leading-tight text-gray-900 md:text-6xl">
          {heading.split(' - ')[0]} {/* Extract the main title part */}
          <br />
          <span className="bg-gradient-to-r from-action to-pink-600 bg-clip-text text-transparent">
            {product ? 'Professional Solution' : 'In The Scrap Bin.'}
          </span>
        </h1>
